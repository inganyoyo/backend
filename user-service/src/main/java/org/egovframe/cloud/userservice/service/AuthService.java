...
public class AuthService {
    ...
    public void initCache() {
        // 사용자 정보 캐시 - 성능 최적화된 설정
        userCache = Caffeine.newBuilder()
            .maximumSize(5000)                                    // 1000 → 5000으로 증가
            .expireAfterWrite(10, TimeUnit.MINUTES)              // 5분 → 10분으로 증가
            .expireAfterAccess(5, TimeUnit.MINUTES)              // 추가: 접근 후 5분 만료
            .recordStats()                                        // 캐시 통계 수집
            .build();
        
        // 세션 존재 여부 캐시 - 더 빠른 응답을 위한 최적화
        sessionExistsCache = Caffeine.newBuilder()
            .maximumSize(10000)                                  // 2000 → 10000으로 증가
            .expireAfterWrite(30, TimeUnit.SECONDS)              // 1분 → 30초 (더 자주 갱신)
            .recordStats()
            .build();
        
        log.info("인증 서비스 캐시 초기화 완료 - userCache: max=5000, ttl=10m/5m, sessionCache: max=10000, ttl=30s");
    }
    ...
    public User getUser(String sessionId) {
        ...
        try {
            ...
            // 캐시에 저장 (즉시) - 다음 요청 시 빠른 응답
            userCache.put(sessionId, user);
            sessionExistsCache.put(sessionId, true);
            
            // 완전 비동기로 Redis TTL 연장 (성능 향상)
